...
def process_pid(pid):
    ...
    return render_template('pid.html', data=process_one, \
        threadlist=current_app.curr_device.g_threads.thread_dict)
...
def process():
    ...
    return render_template('task_info.html', data=pslist, \
        ordered=current_app.curr_device.g_threads.ordered, isthread=0, num=len(pslist))
...
def process_diffcpu():
    ...
    for key in current_app.curr_device.g_threads.process_dict:
        if key in ps_new.process_dict:
            ps.append([current_app.curr_device.g_threads.process_dict[key].stat[1], \
            current_app.curr_device.g_threads.process_dict[key].stat[40],\
...
def process_diffmem():
    ...
    for key in current_app.curr_device.g_threads.process_dict:
        if key in ps_new.process_dict and ps_new.process_dict[key].VmPeak:
            ...
            if diff > 0:
                ps.append([current_app.curr_device.g_threads.process_dict[key].stat[1]+\
                '('+str(current_app.curr_device.g_threads.process_dict[key].stat[0])+')', \
                1, diff])
            elif diff < 0:
                ps.append([current_app.curr_device.g_threads.process_dict[key].stat[1]+\
                '('+str(current_app.curr_device.g_threads.process_dict[key].stat[0])+')', \
...
def process_monitor():
    return render_template('task_monitor.html', cpunum=current_app.curr_device.g_cpunum, \
        processes=current_app.curr_device.g_processes)
...
def thread():
    ...
    current_app.curr_device.g_threads.ordered = sorted(pslist.keys())
    return render_template('task_info.html', data=pslist, \
        ordered=current_app.curr_device.g_threads.ordered, isthread=1, num=len(pslist))
...
def thread_diffcpu():
    ...
    for key in current_app.curr_device.g_threads.thread_dict:
        if key in ps_new.thread_dict:
            ps.append([current_app.curr_device.g_threads.thread_dict[key].stat[1], \
            current_app.curr_device.g_threads.thread_dict[key].stat[40], \
...
def update_pid():
    ...
    if hasattr(current_app, 'curr_device'):
        threadutime = {}
        threadstime = {}
        ps = Process(current_app.curr_device.zclient, pid=int(request_id))
        ps.getstat()
        ret = ps.getstatus()
        if ps.nr_threads > 1:
            ps.getthreads()
