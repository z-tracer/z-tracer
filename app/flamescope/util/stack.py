...
def get_stack_list():
    all_files = []
    print(join(current_app.config['STACK_DIR']))
    for root, dirs, files in walk(join(current_app.config['STACK_DIR'])):
        start = root[len(current_app.config['STACK_DIR']) + 1:]
        for f in files:
            if not f.startswith('.'):
...
def calculate_stack_range(filename):
    ...
    path = current_app.config['STACK_DIR'] + '/' + filename
...
def generate_stack(filename, range_start=None, range_end=None):
    path = join(current_app.config['STACK_DIR'], filename)
    # ensure the file is below STACK_DIR:
    if not abspath(path).startswith(abspath(current_app.config['STACK_DIR'])):
